...
def   wr( fd, req ) :
  ...
  if( address     >= args.block_num ) :
    logging.info('address out of range')
    return [ ACK_FAIL ]
  if( len( data ) != args.block_len ) :
    logging.info('data longer thab block length')
    return [ ACK_FAIL ]
  ...
  if( len( data ) != n              ) :
    logging.info('failed to write fully')
    return [ ACK_FAIL ]
...
if ( __name__ == '__main__' ) :
  ...
  while ( True ) :
    ...
    if   ( req[ 0 ] == REQ_CONF ) :
      ...
      logging.info('code {} not recognised'.format(req[0]))
      ack = [ ACK_FAIL ]
